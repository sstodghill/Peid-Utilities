...
public class Base64UrlEncoder
{
    ...
    private static byte encoding64[] =
    {
    ...
    private static byte encodingUrl[] =
    {
        (byte) 'A', (byte) 'B', (byte) 'C', (byte) 'D', (byte) 'E', (byte) 'F', (byte) 'G', (byte) 'H', // 0-7
        (byte) 'I', (byte) 'J', (byte) 'K', (byte) 'L', (byte) 'M', (byte) 'N', (byte) 'O', (byte) 'P', // 8-15
        (byte) 'Q', (byte) 'R', (byte) 'S', (byte) 'T', (byte) 'U', (byte) 'V', (byte) 'W', (byte) 'X', // 16-23
    ...
    public void process() throws IOException
    {
        ...
        int off = 0;
        int count = 0;
        byte encoding[] = (encodeUrl) ? encodingUrl : encoding64;
     ...
     */
    public Base64UrlEncoder(byte[] bytes)
    {
        this.stringp = true;
     ...
     */
    public Base64UrlEncoder(String input)
    {
        byte bytes[];
        try
        {
     ...
     */
    public Base64UrlEncoder(InputStream in, OutputStream out)
    {
        this.in = in;
        this.out = out;
        this.stringp = false;
    ...
    public static void main(String args[])
    {
        if (args.length ==0)
        {
            System.out.println("Base64UrlEncoder [-p | -u | -pu] <string>");
            System.out.println(" p=>drop padding, u=>encode URL form, neither=>both off");
            System.exit(0);
        }
        String target;
        Base64UrlEncoder b = null;
        if (args.length == 2) {
            b = new Base64UrlEncoder(args[1]);
            if (args[0].equalsIgnoreCase("-p"))
            {
                ...
                System.out.println("Invalid args, use none to get usage");
            }
        } else {
            b = new Base64UrlEncoder(args[0]);
        }
        System.out.println("[" + b.processString() + "]");
        // joe:eoj -> am9lOmVvag==
        //p joe:eoj -> am9lOmVvag
